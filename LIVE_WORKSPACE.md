...
Web research/citations, attachment content ingestion, interactive human approval checkpoints, tool execution, full profile synchronization, editable server artifacts, and live branching are not wired into this service. The live composer captures bounded file/image metadata and clearly tells the advisory model that file contents are unavailable. Council verdicts are persisted and clearly marked as advisory; the richer approval and artifact interactions remain in the labeled design preview. Live conversation storage is centralized server persistence, not offline conflict-resolving synchronization. The roster catalog and college microservice are new module boundaries, but they do not claim that every legacy pipeline has been migrated.
