...
from institution.registry import CATALOG, run_specialist, select
...
def test_every_specialist_uses_its_own_prompt(agent_id):
    async def provider(messages):
        assert messages[0]["content"].startswith(
            CATALOG[agent_id].system_prompt
        )
    ...
    assert (
        asyncio.run(
            run_specialist(
                agent_id, [{"role": "user", "content": "Review"}], provider
            )
        )
        == "Advisory result"
    )
...
def test_identity_role_is_not_read_from_user_metadata(client, monkeypatch):
    ...
    async def verified(*args, **kwargs):
        return httpx.Response(
            ...
            json={
                ...
                "user_metadata": {"aetherion_role": "admin"},
            },
        )
    ...
    result = client.post(
        "/api/auth/identity/exchange",
        json={"access_token": "verified-test-token"},
    ).json()
