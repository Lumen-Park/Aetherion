...
def request_from(peer, forwarded=""):
    return Request({"type": "http", "client": (peer, 12345),
...
def test_untrusted_headers_do_not_select_the_region(monkeypatch):
    ...
    assert experience.client_address(request_from("8.8.8.8", "1.1.1.1")) == "8.8.8.8"
    assert experience.client_address(request_from("127.0.0.1", "1.1.1.1")) is None


def test_proxy_walk_stops_at_first_untrusted_hop(monkeypatch):
    monkeypatch.setenv("AETHERION_TRUSTED_PROXY_CIDRS", "10.1.0.0/24")
    assert experience.client_address(request_from("10.1.0.2", "9.9.9.9, 8.8.8.8, 10.1.0.3")) == "8.8.8.8"


@pytest.mark.parametrize("address", ["127.0.0.1", "10.0.0.1", "::1", "not-an-ip"])
...
def test_unavailable_database_does_not_block_entry(monkeypatch):
    ...
    with TestClient(app, client=("8.8.8.8", 12345)) as client:
        assert client.get("/experience/context").json() == {"country": None, "source": "unavailable"}
