...
const API_BASE = `${(import.meta.env.VITE_API_ORIGIN || "").replace(/\/$/, "")}/api`;
...
export const agentsAPI = {
  ...
  institutionCatalog: () => apiClient.get("/institution/catalog"),
};
...
export const connectDeliberationSocket = (onMessage) => {
  ...
  const ws = new WebSocket(
    `${protocol}//${window.location.host}/api/ws/deliberation`,
  );
  ws.onmessage = (event) => onMessage(JSON.parse(event.data));
  ws.onclose = () =>
    setTimeout(() => connectDeliberationSocket(onMessage), 3000);
  return ws;
};
